...
class movie:
    ...
    def get_movie(self, genre):
        top = ia.get_top50_movies_by_genres(genre)
        top[0]
        print('Title recommended:',top[0])
...
# in that genre.
movie_input = ia.search_movie_advanced(input('Ingresa el título de una película que te encante: '))

def movie_recommend(movie_input,movie_dict):
    ia.search_movie(movie_input)

    movie = movie()
    movie.get_movie('romance')
